...
const validateInput = (event) => {
    const regex = /^[a-zA-Z0-9 ._-]*$/;  // Define allowed characters

    // Check if the current input matches the regex
    if (!regex.test(event.target.value)) {
        event.preventDefault();  // Prevent input if invalid character is typed
    }
};
</script>
